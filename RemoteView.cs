 ...
 * RemoveView: A camera looking at a GameObject, rendering to a texture.
 * 
	...
	public class RemoteView {
		...
		public void setup(int w, int h, GameObject focus) {
			if(edgeDetectShader == null) {
				...
				for(int i=0; i<256; ++i) t.SetPixel(i, 0, Color.Lerp(Color.black, XKCDColors.Amber, i/1024f));
				for(int i=0; i<10; ++i) t.SetPixel(i, 0, XKCDColors.Amber);
				t.Apply();
			...
			if(lookat != focus) {
				...
				foreach(Collider c in focus.GetComponentsInChildren<Collider>()) {
					if(c.bounds != null) bounds.Encapsulate(c.bounds);
				}
		...
		public void updateCamera() {
			if(updateFrame > Time.frameCount - 5) return;
			...
			if(lookdetail != null) pos_target = lookdetail.transform.position;
